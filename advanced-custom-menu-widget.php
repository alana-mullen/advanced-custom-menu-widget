<?php
/*
  Plugin Name: Advanced Custom Menu Widget
  Description: Add a custom menu as a widget with options for adding classes and attributes.
...
if ( ! class_exists( 'advanced_custom_menu_widget' ) ) {

	// Load the widget
	add_action( 'widgets_init', 'load_advanced_custom_menu_widget' );

	// Register the widget
	function load_advanced_custom_menu_widget() {
		register_widget( 'advanced_custom_menu_widget' );
	}

	/**
	 * Advanced Custom Menu Widget class
	 */
	class advanced_custom_menu_widget extends WP_Widget {

		function __construct() {
			load_plugin_textdomain( 'advanced-custom-menu-widget', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
			$widget_ops  = array(
				'classname'   => 'advanced-custom-menu-widget',
				'description' => __( 'Add a custom menu as a widget with options for adding classes and attributes.', 'advanced-custom-menu' )
			);
			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'advanced-custom-menu-widget' );
			parent::__construct( 'advanced-custom-menu-widget', __( 'Advanced Custom Menu', 'advanced-custom-menu-widget' ), $widget_ops, $control_ops );
		}
		...
		public function form( $instance ) {
			...
			if ( ! $menus ) {
				echo '<p>' . sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'advanced-custom-menu' ), admin_url( 'nav-menus.php' ) ) . '</p>';
			...
			<p><label
					for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'advanced-custom-menu' ) ?></label><input
					type="text" class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>"
					name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo esc_html( $title ); ?>"/>
			</p>
			<p><label
					for="<?php echo $this->get_field_id( 'title_url' ); ?>"><?php _e( 'Title URL:', 'advanced-custom-menu' ) ?></label><input
			...
			<p><label
					for="<?php echo $this->get_field_id( 'nav_menu' ); ?>"><?php _e( 'Select Menu:', 'advanced-custom-menu' ); ?></label>
			...
			<p><label
					for="<?php echo $this->get_field_id( 'menu_class' ); ?>"><?php _e( 'Menu Classes:', 'advanced-custom-menu' ) ?></label><input
				...
				<small><?php _e( 'CSS classes to use for the ul menu element. Separate classes by a space.', 'advanced-custom-menu' ); ?></small>
			</p>
			<p><label
					for="<?php echo $this->get_field_id( 'menu_attr' ); ?>"><?php _e( 'Menu Attributes:', 'advanced-custom-menu' ) ?></label><input
				...
				<small><?php _e( 'Attributes to use for the ul menu element. Separate attributes by a space.', 'advanced-custom-menu' ); ?></small>
			</p>
